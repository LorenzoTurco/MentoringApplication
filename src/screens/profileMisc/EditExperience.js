...
export default EditExperience = ({route, navigation}) =>{
    ...
    let [newFromMonth, setNewFromMonth] = useState(exp.monthFrom);
    let [newToMonth, setNewToMonth] = useState(exp.monthTo);
    let [newFromYr, setNewFromYr] = useState(exp.fromYr);
    let [newToYr, setNewToYr] = useState(exp.toYr);
    let id = exp.id;
    let [newTitle, setNewTitle ]  = useState(exp.title);
    let [newDesc, setNewDesc] = useState(exp.desc);
